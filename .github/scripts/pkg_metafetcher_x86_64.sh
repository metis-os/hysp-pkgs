...
PKG_VERSION="$(echo $RELEASE_METADATA | jq -r '.tag_name' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export PKG_VERSION="$PKG_VERSION"
PKG_RELEASED="$(echo $RELEASE_METADATA | jq -r '.published_at' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export PKG_RELEASED="$PKG_RELEASED"
REPO_URL="$(echo $REPO_METADATA | jq -r '.html_url' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export REPO_URL="$REPO_URL"
SIZE="$(echo $PKG_METADATA | jq -r '.size' | awk '{printf "%.2f MB\n", $1 / (1024 * 1024)}' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export SIZE="$SIZE"
SHA="$(echo "$SHA256_SUMS" | grep -i "x86_64/$BIN$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/^[ \t]*//;s/[ \t]*$//')" && export SHA="$SHA"
SOURCE_URL="$(echo $PKG_METADATA | jq -r '.download_url' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export SOURCE_URL="$SOURCE_URL"
STARS="$(echo $REPO_METADATA | jq -r '.stargazers_count' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export STARS="$STARS"
