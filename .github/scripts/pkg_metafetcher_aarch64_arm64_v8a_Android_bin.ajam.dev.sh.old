...
# export SOURCE_BIN="https://bin.ajam.dev/arm64_v8a_Android" #ROOT Dir for bins

##Usage: 
# Actions: BIN="$BIN" REPO="$REPO" SOURCE_BIN="https://bin.ajam.dev/arm64_v8a_Android" bash "$GITHUB_WORKSPACE/main/.github/scripts/pkg_metafetcher_aarch64_arm64_v8a_Android_bin.ajam.dev.sh"
# General: BIN="$BIN" REPO="$REPO" SOURCE_BIN="https://bin.ajam.dev/arm64_v8a_Android" bash <(curl -qfsSL "https://raw.githubusercontent.com/metis-os/hysp-pkgs/main/.github/scripts/pkg_metafetcher_aarch64_arm64_v8a_Android_bin.ajam.dev.sh")
#List all available $BIN: curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/METADATA.json" | jq -r '.[].Name' | sort -u

#List NAME:SIZE format
# curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/METADATA.json" | jq -r 'map("\(.Name):\(.Size)") | .[]' | sort -u
...
BSUM="$(echo "$B3_SUMS" | grep -i "\.\/$BIN$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export BSUM="$BSUM"
SHA="$(echo "$SHA256_SUMS" | grep -i "\.\/$BIN$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//')" && export SHA="$SHA"
