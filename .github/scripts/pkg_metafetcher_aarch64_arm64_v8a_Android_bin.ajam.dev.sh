...
curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/METADATA.json" | jq --arg BIN "$BIN" --arg REPO_URL "$REPO_URL" --arg WEB_URL "$WEB_URL" '.[] | select(.name == $BIN) | if .web_url == $WEB_URL then . else if .repo_url == $REPO_URL then . else empty end end' > "$TMPDIR/METADATA.json"
#-------------------------------------------------------#
