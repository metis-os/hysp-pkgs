...
# bash "$GITHUB_WORKSPACE/main/.github/scripts/pkg_metaupdater_aarch64_arm64.sh"
...
# This works
#envsubst < "$GITHUB_WORKSPACE/main/data/aarch64_arm64/eget.toml"
#awk -v STARS="$STARS" '/stars/{gsub(/\$STARS/, STARS)}1' "$GITHUB_WORKSPACE/main/data/aarch64_arm64/eget.toml"
#This also works:
...
if [ ! -s "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml" ]; then
   #Get the sample
   curl -qfsSL "https://raw.githubusercontent.com/metis-os/hysp-pkgs/main/data/SPEC/SAMPLE_SPEC_aarch64_arm64.toml" -o "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
   chmod +xwr "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
   dos2unix --quiet "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
fi
...
awk -v BIN="$BIN" -v DESCRIPTION="$DESCRIPTION" -v AUTHOR="$AUTHOR" -v REPO_URL="$REPO_URL" -v STARS="$STARS" -v PKG_VERSION="$PKG_VERSION" -v PKG_RELEASED="$PKG_RELEASED" -v SIZE="$SIZE" -v BSUM="$BSUM" -v SHA="$SHA" -v SOURCE_URL="$SOURCE_URL" -v LANGUAGE="$LANGUAGE" -v LICENSE="$LICENSE" -v TOPICS="$TOPICS" '{gsub(/name = .*/, "name = \"" BIN "\""); \
      ...
      print }' "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml" | envsubst | sponge "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
##Just In Case      
#awk -v STARS="$STARS" '/stars/{gsub(/\$STARS/, STARS)}1' "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml" | sponge "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
#Temp Fix:
sed '/^description = / s/.*"\(.*\)".*/description = "\1"/' -i "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml"
#Print
echo -e "\n [+] $GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml\n" && cat "$GITHUB_WORKSPACE/main/data/aarch64_arm64/$BIN.toml" && echo -e "\n"    
